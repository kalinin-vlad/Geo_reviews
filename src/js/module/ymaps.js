...
function mapInit() {
  ymaps.ready(() => {
    ...
    clusterer = new ymaps.Clusterer({ clusterDisableClickZoom: true, preset: 'islands#blueClusterIcons' });
...
function getGeoObjects(map) {
  const geoObjects = [];
  for (const review of getReviewsFromLS() || []) {
    const placemark = new ymaps.Placemark((review.coords), {}, { preset: 'islands#greenIcon' });
